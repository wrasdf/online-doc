...
@pytest.mark.asyncio
async def test_share_document(client: AsyncClient) -> None:
    """
    ...
    async with TestingSessionLocal() as session:
        ...
        user_to_share_with, _ = await create_random_user(session, username="shareduser", email="shared@example.com")
...
@pytest.mark.asyncio
async def test_share_document_not_owner(client: AsyncClient) -> None:
    """
    Test that a non-owner cannot share a document.
    """
    async with TestingSessionLocal() as session:
        # Create owner and a document
        owner, _ = await create_random_user(session)
        document = await create_random_document(session, owner_id=owner.id)

        # Create a non-owner user
        non_owner, non_owner_headers = await create_random_user(session, username="nonowner", email="nonowner@example.com")
...
@pytest.mark.asyncio
async def test_share_document_not_found(client: AsyncClient) -> None:
    """
